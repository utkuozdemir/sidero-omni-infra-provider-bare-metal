...
func (m *MachineSpec) CloneVT() *MachineSpec {
	...
	r := new(MachineSpec)
	r.IpmiIp = m.IpmiIp
	r.IpmiPort = m.IpmiPort
	r.IpmiPassword = m.IpmiPassword
...
func (this *MachineSpec) EqualVT(that *MachineSpec) bool {
	...
	if this.IpmiIp != that.IpmiIp {
		return false
	}
	if this.IpmiPort != that.IpmiPort {
		return false
	}
	if this.IpmiPassword != that.IpmiPassword {
		return false
	}
...
func (m *MachineSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.IpmiPassword) > 0 {
		i -= len(m.IpmiPassword)
		copy(dAtA[i:], m.IpmiPassword)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.IpmiPassword)))
		i--
		dAtA[i] = 0x1a
	}
	if m.IpmiPort != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.IpmiPort))
		i--
		dAtA[i] = 0x10
	}
	if len(m.IpmiIp) > 0 {
		i -= len(m.IpmiIp)
		copy(dAtA[i:], m.IpmiIp)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.IpmiIp)))
		i--
		dAtA[i] = 0xa
	}
...
func (m *MachineSpec) SizeVT() (n int) {
	...
	if m.IpmiPort != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.IpmiPort))
	}
...
func (m *MachineSpec) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field IpmiIp", wireType)
			}
			...
			m.IpmiIp = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field IpmiPort", wireType)
			}
			m.IpmiPort = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.IpmiPort |= uint32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
